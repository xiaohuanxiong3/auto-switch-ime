package com.sqy.plugins.translate.ui
...
import com.sqy.plugins.translate.engine.BaiduTranslateEngine
import com.sqy.plugins.translate.service.TranslationUIManager
import com.sqy.plugins.translate.ui.icon.Spinner
import java.awt.*
...
//        translate(Lang.AUTO, targetLang)
        // TODO 当前为了跑通流程没有进行封装，后续需要进行封装设计
        if (text.isBlank()) {
            return
        }
        ...
        check(ApplicationManager.getApplication().isDispatchThread) {
            "${TranslationBalloon::class.java.simpleName} must only be used from the Event Dispatch Thread."
        }
        ...
        application.executeOnPooledThread {
            try {
                val translator = BaiduTranslateEngine()
                with(translator) {
                    translate(text).let {
                        application.invokeLater {
                            showTranslation(it,false)
                        }
                    }
                }
            } catch (error : Throwable) {
                val log = Logger.getInstance(TranslationBalloon::class.java)
                log.error("翻译错误",error)
                application.invokeLater {
                    showError(error)
                }
            }
